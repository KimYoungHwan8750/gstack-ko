...
description: |
  ...
  작업 주간이나 스프린트 마무리 시 선제적으로 제안하세요.
allowed-tools:
  - Bash
  - Read
  - Write
...
find ~/.gstack/projects/$SLUG -name '*-reviews.jsonl' -type f -exec cat {} + 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
