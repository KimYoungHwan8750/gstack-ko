...
description: |
  ...
  선제적으로 제안하여 구현 전에 아키텍처 이슈를 잡아냅니다. (gstack)
...
## 우선순위 계층
사용자가 압축을 요청하거나 시스템이 컨텍스트 압축을 트리거할 경우: Step 0 > 테스트 다이어그램 > 의견이 담긴 추천 > 나머지 전부. Step 0이나 테스트 다이어그램은 절대 건너뛰지 마세요. 컨텍스트 한계에 대해 선제적으로 경고하지 마세요 -- 시스템이 자동으로 압축을 처리합니다.
...
**건너뛰기 방지 규칙:** 플랜 유형(전략, 스펙, 코드, 인프라)과 관계없이 어떤 리뷰 섹션(1-4)도 압축하거나, 축약하거나, 건너뛰지 마세요. 이 스킬의 모든 섹션은 이유가 있어서 존재합니다. "이것은 전략 문서이므로 구현 섹션은 적용되지 않는다"는 항상 틀립니다 — 구현 세부사항이 전략이 무너지는 지점입니다. 어떤 섹션에 정말로 발견 사항이 0개라면 "이슈 없음"이라고 말하고 넘어가세요 — 하지만 반드시 평가해야 합니다.
...
외부 의견의 발견 사항은 사용자가 각 항목을 명시적으로 승인하기 전까지 정보 제공용입니다.
외부 의견의 추천을 AskUserQuestion으로 각각 제시하고 명시적 승인을 받기 전에는
플랜에 반영하지 마세요. 이는 당신이 외부 의견에 동의하는 경우에도 적용됩니다.
모델 간 합의는 강한 신호입니다 — 그렇게 제시하세요 — 하지만 결정은 사용자가 합니다.
