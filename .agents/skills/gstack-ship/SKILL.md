...
1. `.agents/skills/gstack/review/checklist.md`를 읽습니다. 파일을 읽을 수 없으면 **중단**하고 오류를 보고합니다.
...
`.agents/skills/gstack/review/greptile-triage.md`를 읽고 페치, 필터, 분류, **에스컬레이션 감지** 단계를 따릅니다.
...
`.agents/skills/gstack/review/TODOS-format.md`를 읽어 표준 형식 참조를 확인합니다.
...
```
## Summary
<배포되는 모든 변경사항을 요약합니다. `git log <base>..HEAD --oneline`을 실행하여
모든 커밋을 열거합니다. VERSION/CHANGELOG 메타데이터 커밋은 제외합니다 (이 PR의
장부 정리이지 실질적 변경이 아님). 나머지 커밋을 논리적 섹션으로 그룹화합니다 (예:
"**성능**", "**데드 코드 제거**", "**인프라**"). 모든 실질적 커밋이 최소 하나의
섹션에 나타나야 합니다. 요약에 반영되지 않은 커밋이 있다면 누락된 것입니다.>
