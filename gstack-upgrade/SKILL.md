...
```
사용자에게 전달합니다: "업데이트 확인이 비활성화되었습니다. 다시 활성화하려면 `~/.claude/skills/gstack/bin/gstack-config set update_check true`를 실행하세요."
현재 스킬을 계속 진행합니다.
...
### Step 4.5: 로컬 벤더 복사본 동기화

Step 2의 설치 디렉토리를 사용합니다. 업데이트가 필요한 로컬 벤더 복사본이 있는지 확인합니다:
...
`LOCAL_GSTACK`가 비어 있지 않으면, 방금 업그레이드한 기본 설치에서 복사하여 업데이트합니다 (README 벤더 설치와 동일한 방식):
```bash
...
```
사용자에게 전달합니다: "`$LOCAL_GSTACK`의 벤더 복사본도 업데이트되었습니다 — 준비되면 `.claude/skills/gstack/`을 커밋하세요."
...
위의 Step 2 bash 블록을 실행하여 기본 설치 유형과 디렉토리(`INSTALL_TYPE` 및 `INSTALL_DIR`)를 감지합니다. 그런 다음 위의 Step 4.5 감지 bash 블록을 실행하여 로컬 벤더 복사본(`LOCAL_GSTACK`)을 확인합니다.

**`LOCAL_GSTACK`가 비어 있는 경우** (로컬 벤더 복사본 없음): 사용자에게 "이미 최신 버전(v{version})입니다."라고 전달합니다.

**`LOCAL_GSTACK`가 비어 있지 않은 경우**, 버전을 비교합니다:
```bash
...
**버전이 다른 경우:** 위의 Step 4.5 동기화 bash 블록을 따라 기본 설치에서 로컬 복사본을 업데이트합니다. 사용자에게 전달합니다: "전역 v{PRIMARY_VER}은 최신입니다. 로컬 벤더 복사본을 v{LOCAL_VER} → v{PRIMARY_VER}로 업데이트했습니다. 준비되면 `.claude/skills/gstack/`을 커밋하세요."
