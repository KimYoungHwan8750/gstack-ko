...
description: |
  ...
  site", "take a screenshot", "dogfood this". (gstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
## CSS Inspector 및 스타일 수정

### 요소 CSS 검사
```bash
$B inspect .header              # selector의 전체 CSS cascade
$B inspect                      # 사이드바에서 마지막으로 선택한 요소
$B inspect --all                # user-agent stylesheet 규칙 포함
$B inspect --history            # 수정 히스토리 표시
```

### 스타일 실시간 수정
```bash
$B style .header background-color #1a1a1a   # CSS 속성 수정
$B style --undo                              # 마지막 변경 되돌리기
$B style --undo 2                            # 특정 변경 되돌리기
```

### 깔끔한 스크린샷
```bash
$B cleanup --all                 # ads, cookies, sticky, social 제거
$B cleanup --ads --cookies       # 선택적 cleanup
